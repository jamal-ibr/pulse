...
export interface StreamReplyOptions {
  ...
  /** Turn-scoped instruction, e.g. an in-progress transfer handoff. */
...
export async function streamReply(options: StreamReplyOptions): Promise<string> {
  const { callId, transcript, isReminder, availabilityContext, turnInstruction, signal, onDelta } =
    options;
  const messages = transcriptToMessages(transcript);

  // Per-call and per-turn context goes after the stable prompt.
  const system = [SYSTEM_PROMPT, availabilityContext, turnInstruction]
    .filter(Boolean)
    .join("\n\n");
  ...
  try {
    ...
    const fallback = "Sorry, I didn't quite catch that. Could you say that again for me, please?";
    onDelta(fallback);
    return fallback;
  }
}
